...
import { RoomSharp, LinearScale } from '@material-ui/icons'
...
class DrawProjectPage extends React.Component {
  ...
  _renderMarkerinNewPoint = () => {
    ...
    if (markerLng && markerLat) {
      return (
        <Marker longitude={markerLng} latitude={markerLat} offset={[0, -15]}>
          <RoomSharp
            style={{
              color: 'green',
              fontWeight: 'bold',
    ...
    return (
      <React.Fragment>
        ...
        >
          ...
          <Source id="river" type="geojson" data={this.props.riverData} />
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    replaceCoordinatesProject: (features) =>
      dispatch(replaceCoordinatesProjectAction(features)),
    insertProjectFeatures: (features) =>
      dispatch(insertProjectFeaturesAction(features)),
