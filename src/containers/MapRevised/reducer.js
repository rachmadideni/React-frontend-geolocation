...
export const initialState = fromJS({
  ...
  map: {
    ...
    accessToken:
      'pk.eyJ1IjoiZGVuaXJhY2htYWRpIiwiYSI6ImNqdXptYTVoMzFhZWs0ZnMwbmI3dG00eWgifQ.tkFYtFMZwzujEkvtz9_Oiw',
  },
  error: {
    message: null,
  },
  ...
  switch (action.type) {
    ...
    case GET_PROJECT_SUCCESS_ACTION:
      return state
        ...
        .setIn(['data', 'proyek', 'features'], new List(action.payload))
    ...
    case GET_RIVER_SUCCESS_ACTION:
      return state
        .setIn(['ui', 'loading'], false)
        .setIn(['data', 'sungai', 'features'], new List(action.payload))
    ...
    case GET_PROJECT_FILES_SUCCESS_ACTION: {
      return state
        .setIn(['ui', 'loading'], false)
        .setIn(['data', 'gambar_proyek', 'files'], new List(action.payload))
    }
    case GET_PROJECT_ATTRIBUTE_FAIL_ACTION:
      return state.setIn(
        ['data', 'attribute', 'proyek'],
